...
class ProductListView(generics.ListAPIView):
    ...
    @method_decorator(
        cache_page(60 * 10)
    )  # Cache for 10 minutes - cache key based on query params
            ...
            data.append(
                {
                    ...
                    "image": avatar_url,
                    "sample_products": ProductSerializer(
...
def home_view(request):
    ...
    products = Product.objects.filter(is_active=True, stock_quantity__gt=0).order_by(
        "-created_at"
    )[:12]
    context = {
...
def artisan_detail_view(request, artisan_id):
    artisan_profile = get_object_or_404(
        ArtisanProfile, artisan__id=artisan_id, profile_verified=True
    )
        ...
        try:
            if artisan_profile.artisan.avatar:
                data["image"] = request.build_absolute_uri(
                    artisan_profile.artisan.avatar.url
                )
        except Exception:
            data["image"] = None
