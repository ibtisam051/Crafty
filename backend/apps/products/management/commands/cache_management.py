...
class Command(BaseCommand):
    ...
    def clear_cache(self, cache_name):
        ...
        if cache_name == 'all':
            for name in ['default', 'session', 'products']:
                caches[name].clear()
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Cleared {name} cache')
                )
        else:
            caches[cache_name].clear()
            self.stdout.write(
                self.style.SUCCESS(f'✓ Cleared {cache_name} cache')
            )
    ...
    def clear_products_cache(self):
        ...
        self.stdout.write(
            self.style.SUCCESS('✓ Cleared all product-related caches')
        )
    ...
    def clear_sessions_cache(self):
        ...
        self.stdout.write(
            self.style.SUCCESS('✓ Cleared session cache')
        )
    ...
    def show_info(self):
        ...
        self.stdout.write("  python manage.py cache_management --action clear --cache products")
        self.stdout.write("  python manage.py cache_management --action stats --cache default")
