...
from apps.users.models import User, ArtisanProfile
...
for cat_data in categories_data:
    ...
    print(f'Created category: {cat.name}' if created else f'Category exists: {cat.name}')
...
for art_data in artisans_data:
    ...
    print(f'Created artisan profile: {profile.business_name}' if created else f'Artisan exists: {profile.business_name}')
    # Attach an avatar if a default artist image exists
    try:
        media_avatars_dir = pathlib.Path(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'media', 'avatars'))
...
for i, prod_data in enumerate(products_data):
    ...
    try:
        ...
        media_products_dir = pathlib.Path(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'media', 'products'))
        if media_products_dir.exists():
            ...
            if match:
                ...
                pi, picreated = ProductImage.objects.get_or_create(
                    product=product,
                    defaults={'image': image_rel_path, 'is_primary': True, 'sort_order': 0}
                )
                print(f'Attached image {image_rel_path} to product {product.name}' if picreated else f'Image exists for {product.name}')
    except Exception as e:
