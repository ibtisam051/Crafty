...
export const AuthProvider = ({ children }) => {
  const [token, setToken] = useState(() => {
    ...
    console.log(
      "[AuthContext] Initializing token from localStorage:",
      t?.substring(0, 50),
    );
    return t;
  });
  const [refreshToken, setRefreshToken] = useState(() => {
    ...
    console.log(
      "[AuthContext] Initializing refreshToken from localStorage:",
      rt?.substring(0, 50),
    );
    return rt;
  });
  const [user, setUser] = useState(() => {
    try {
      ...
      console.log(
        "[AuthContext] Initializing user from localStorage:",
        u?.username,
      );
      return u;
    } catch (e) {
